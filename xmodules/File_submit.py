...
import requests
import json
import xutils.make_submit_params as make_params
# from ..xutils import Analyze_constant
import xutils.Analyze_constant as Analyze_constant
...
class Filesubmit():
    ...
    def post_response(self, file_name, file_dir):
        ...
        # 发送post请求
        response = requests.post(url=self.baseurl, data=make_params.main(), files=files)
        return response
    ...
    def saversp(self, key, value):
        ...
        # 保存文件
        tf = open("D:\IDEA\PyCharm 2023.2\demo\X-API\\xutils\sha256.json", "w")
